...
class Climate_EfficientNet_2D(nn.Module): 
    
    def __init__(self,num_channels,layers,hidden_size):
        super().__init__()
        layers_cnn = []
        activation_fns = []
        self.block = EfficientNet
        self.inplanes = num_channels
...
class ClimODE_uncertain_region_EfficientNet(nn.Module): 
    ...
    def __init__(self,num_channels,const_channels,out_types,method,use_att,use_err,use_pos):
        ...
        self.hidden = [128,64,2*out_types]
        input_channels = 30 + out_types*int(use_pos) + 34*(1-int(use_pos))
        self.vel_f = Climate_EfficientNet_2D(input_channels,self.layers,self.hidden)
        ...
        self.method = method
        err_in =  9 + out_types*int(use_pos) + 34*(1-int(use_pos))
        if use_err: self.noise_net = Climate_EfficientNet_2D(err_in,[3,2,2],[128,64,2*out_types])
        if use_pos: self.pos_enc = Climate_EfficientNet_2D(4,[2,1,1],[32,16,out_types])
        self.att = use_att
        self.err = use_err
        self.pos = use_pos
        self.pos_feat = 0
